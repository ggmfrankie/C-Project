...
bool PhysicsSystem::isInsideV2(const SoftBody2D &a,
                               const ggm::Vector2f &point) {
  using namespace ggm;
  using vec2 = Vector2f;
  ...
  for (int i = 0; i < length; ++i) {
    ...
    if (const bool yCheck = (p0.y > point.y) != (p1.y > point.y); !yCheck)
      continue;
    if (const float xIntersect =
            (p1.x - p0.x) * (point.y - p0.y) / (p1.y - p0.y) + p0.x;
        point.x < xIntersect)
      inside = !inside;
  }
  return inside;
...
bool PhysicsSystem::isInside(SoftBody2D &a, const ggm::Vector2f &point) {
  ...
  for (int i = 0; i < length; ++i) {
    ...
    if (p0.y >= p1.y) {
      if (std::abs(point.y - p0.y) < EPS || cross < 0)
        continue;
      intersections++;
    } else {
      if (std::abs(point.y - p1.y) < EPS || cross > 0)
        continue;
      intersections++;
    }
  }
...
PhysicsSystem::Collision PhysicsSystem::getCollision(SoftBody2D &body,
                                                     const ggm::Vector2f &p) {
  using namespace ggm;
  using vec2 = Vector2f;
  ...
  for (u64 i = 0; i < length; i++) {
    ...
    if (const float distSq = vec2::distanceSquared(pointOnLine, p);
        distSq < minDistSq) {
      minDistSq = distSq;
